(function (root, factory) {
  ...
  (function initLogging() {
     ...
     * @public
     */
    dust.log = function(message, type, context) {
      type = type || INFO;
  ...
  dust.render = function(nameOrTemplate, context, callback) {
    var chunk = new Stub(undefined, callback).head;
    try {
  ...
  function load(nameOrTemplate, chunk, context) {
    ...
    if (template) {
      var wrapped = Context.wrap(context, template.templateName);
      chunk.context = wrapped;

      return template(chunk, wrapped);
    } else {
      if (dust.onLoad) {
    ...
    if (filters) {
      for (i = 0, len = filters.length; i < len; i++) {
        ...
        if (name === 's') {
          ...
          dust.log('Invalid filter `' + name + '`', WARN, context);
        }
      }
    }
  ...
  dust.filters = {
    ...
    js: function(value, context) { 
      return dust.escapeJSON(value, context); 
    },
    jp: function(value, context) {
      if (!JSON) {dust.log('JSON is undefined; could not parse `' + value + '`', WARN, context);
        return value;
      } else {
  ...
  Context.prototype._get = function(cur, down) {
    ...
    if (typeof ctx === 'function') {
      ...
      if (ctx === undefined) {
        dust.log('Cannot find reference `{' + down.join('.') + '}` in template `' + this.getTemplateName() + '`', INFO, that);
      }
      return ctx;
    }
  };
  ...
  Context.prototype.push = function(head, idx, len) {
    if(head === undefined) {
      dust.log("Not pushing an undefined variable onto the context", INFO, this);
      return this;
    }
  ...
  Context.prototype.getBlock = function(key) {
    ...
    if (typeof key === 'function') {
      key = key(new Chunk(this), this).data.join('');
    }

    blocks = this.blocks;

    if (!blocks) {
      dust.log('No blocks for context `' + key + '` in template `' + this.getTemplateName() + '`', DEBUG, this);
      return false;
    }
    ...
    dust.log('Malformed template `' + this.getTemplateName() + '` was missing one or more blocks.', WARN, this);
    return false;
  };
  ...
  Context.prototype.resolve = function(body) {
    ...
    chunk = new Chunk(this).render(body, this);
  ...
  function Stub(context, callback) {
    this.context = context;
    this.head = new Chunk(context, this);
  ...
  Stub.prototype.flush = function() {
    ...
    while (chunk) {
      if (chunk.flushable) {
        ...
        dust.log('Rendering failed with error `' + chunk.error + '`', ERROR, this.context);
  ...
  Stream.prototype.flush = function() {
    ...
    while(chunk) {
      if (chunk.flushable) {
        ...
        this.emit('end');
        dust.log('Streaming failed with error `' + chunk.error + '`', ERROR, this.context);
        this.flush = EMPTY_FUNC;
        return;
      } else {
        return;
  ...
  Stream.prototype.emit = function(type, data) {
    ...
    if (!handlers.length) {
      dust.log('Stream broadcasting, but no listeners for `' + type + '`', DEBUG, this.context);
      return false;
    }
  ...
  Stream.prototype.on = function(type, callback) {
    ...
    if(typeof callback !== 'function') {
      dust.log('No callback function provided for `' + type + '` event listener', WARN, this.context);
    } else {
  ...
  Stream.prototype.pipe = function(stream) {
    ...
    if(typeof stream.write !== 'function' ||
      ...
      dust.log('Incompatible stream passed to `pipe`', WARN, this.context);
      return this;
    }
    ...
    .on('data', function(data) {
      ...
      try {
        ...
        dust.log(err, ERROR, that.context);
      }
    })
    .on('end', function() {
      if(destEnded) {
  ...
  function Chunk(context, root, next, taps) {
    this.root = root;
    this.next = next;
    this.data = []; //ie7 perf
  ...
  Chunk.prototype.map = function(callback) {
    var cursor = new Chunk(this.context, this.root, this.next, this.taps),
        branch = new Chunk(this.context, this.root, cursor, this.taps);
    ...
    try {
      ...
      dust.log(err, ERROR, this.context);
      branch.setError(err);
    }
    return cursor;
  };
  ...
  Chunk.prototype.section = function(elem, context, bodies, params) {
    ...
    */
    ...
    dust.log('Section without corresponding key in template `' + context.getTemplateName() + '`', DEBUG, this.context);
    return this;
  };
  ...
  Chunk.prototype.exists = function(elem, context, bodies) {
    ...
    if (!dust.isEmpty(elem)) {
      ...
      dust.log('No block for exists check in template `' + context.getTemplateName() + '`', DEBUG, this.context);
    } else if (skip) {
      return skip(this, context);
    }
    return this;
  ...
  Chunk.prototype.notexists = function(elem, context, bodies) {
    ...
    if (dust.isEmpty(elem)) {
      ...
      dust.log('No block for not-exists check in template `' + context.getTemplateName() + '`', DEBUG, this.context);
    } else if (skip) {
      return skip(this, context);
    }
    return this;
  ...
  Chunk.prototype.helper = function(name, context, bodies, params, auto) {
    ...
    if(dust.helpers[name]) {
      try {
        ...
        dust.log('Error in helper `' + name + '`: ' + err.message, ERROR, this.context);
        return chunk.setError(err);
      }
    } else {
      dust.log('Helper `' + name + '` does not exist', WARN, this.context);
      return chunk;
    }
  };
    ...
    return this.map(function(chunk) {
      thenable.then(function(data) {
        ...
        if(errorBody) {
          ...
          dust.log('Unhandled promise rejection in `' + context.getTemplateName() + '`', INFO, that.context);
          chunk.end();
        }
      });
    });
      ...
      stream
        ...
        .on('error', function error(err) {
          ...
          if(errorBody) {
            ...
            dust.log('Unhandled stream error in `' + context.getTemplateName() + '`', INFO, that.context);
          }
          if(!ended) {
            ended = true;
            chunk.end();
  ...
  Chunk.prototype.capture = function(body, context, callback) {
    return this.map(function(chunk) {
      var stub = new Stub(context, function(err, out) {
        if (err) {
          chunk.setError(err);
        } else {
          callback(out, chunk);
  ...
  dust.escapeJSON = function(o, context) {
    if (!JSON) {
      dust.log('JSON is undefined; could not escape `' + o + '`', WARN, context);
      return o;
    } else {
